...
function Cards() {
  /* Context'den degiskenimizi aldik */
	const weatherContext = useContext(WeatherContext);
	let today = new Date();
	let currentDay = today.getDate();

  /* Tarih donusumu icin bu fonksiyonu kullaniyoruz */
	...
	return (
      ...
      {/* Sehir isminin ve sehirlerin secilebildigi select inputun bulundugu alan */}
			<div className='title'>
				{weatherContext.weatherData.city?.name}
				<div>
					<select onChange={e => weatherContext.setCity(JSON.parse(e.target.value))}>
						{city.map((item, index) => {
							return (
								<option
        ...
        {/* Hava durumu bilgisini gosteren cardlarin bulundugu alan */}
			<div className='cards'>
				{weatherContext.weatherData.list?.map((item, index) => {
					return (
						<div className='card' key={index} style={convertDate(item.dt).slice(0, 2) == currentDay ? { border: "3px solid black" } : {}}>
